...
def trend_model(thetas, t):
    ...
    T = torch.tensor([t ** i for i in range(p)]).float().to(thetas.get_device())
...
class NBeatsModel(nn.Module):
    def __init__(self, config):
        ...
        self.week_em = nn.Embedding(7 + 1, config.date_emb_dim)
        self.id_em = nn.Embedding(1000 +1, config.id_emb_dim)
